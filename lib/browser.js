class CDS {
  ...
  getTableUtils() {
    ...
    if(!this.TableUtils)
      throw Error('TableUtils not loaded')
  ...
  getAppContainer() {
    return this.getSplitShell().getContent()
  ...
  getApplicationContainer() {
    return this.getPages()
      .filter(X=>X.isA('sap.ushell.components.container.ApplicationContainer'))[0];
  }
  ...
  getComponentsById(id) {
    ...
    let result = [];
    for(let elementName in elements)
      if(elements[elementName].getId()===id)
        result.push(elements[elementName]);
    return result;
  }
  ...
  getComponentsByIdRe(res) {
    ...
    for(let elementName in elements) {
      res.forEach( re => {
        if(elements[elementName].getId().match(re))
          result.push(elements[elementName]);
      })
    }
    return result;
  ...
  findListReportTableStandardActionButton(actionName) {
    ...
    return this.getComponents("sap.m.Button")
      .find(X => X.getId()==id);
  }
  ...
  editFieldInLineItemTableInSection(subsectionTitle, columnName, rowNumber, value) {
    ...
    if(columnIndex===-1)
      throw Error(`editFieldInLineItemTableInSection: column ${columnName} not found, avaibale columns: ${columnNames.join(', ')}`);
    // sap.m.Table
  ...
  findObjectPageLineItemTableBySubsectionTitle(subsectionTitle) {
    ...
    if(tables.length!=1) {
      ...
      throw Error(`findObjectPageLineItemTableBySubsectionTitle: subsection ${subsectionTitle} not found, length: ${tables.length}, available: ${available.join(', ')}`);
    }
    return tables[0];
  }
  ...
  selectRowsInLineItemTableHaving(tableName, columnName, value) {
    ...
    if(columnIndex===-1) {
      ...
      throw Error(`selectRowsInLineItemTableHaving - column ${columnName} not found, available: ${JSON.stringify(available)}`);
    }
    let items = table.getItems();
    let values = items
      .map(I => I.getCells()[columnIndex])
    ...
    let rowIndexes = values
      ...
      .filter(X => X>=0);
    rowIndexes.forEach(i => items[i]
        .getMultiSelectControl()
        .fireSelect({selected:true})
    )
  }
  ...
  pressStandardActionButtonForLineItemTable(nameOrTitle, actionName) {
    ...
    if(!action) {
      ...
      throw Error(`pressStandardActionButtonForLineItemTable: action ${actionName} in table ${nameOrTitle} not found, available: ${available.join(', ')}`);
    }
  ...
  upToParentOfType(component, type) {
    ...
    if(component.getMetadata().getName()===type)
      return component;
  ...
  confirmDiscardDraft() {
    ...
    let button = popover.getContent()[0].getItems()
      .filter(X => X.getMetadata().getName()=="sap.m.Button")
      .filter(X => X.getText()==="Discard")[0];
    if(!button)
      throw Error("confirmDiscardDraft: button not found");
    button.firePress();
  }
  ...
  pressGoBackButton() {
    let buttons = this.getComponents('sap.ushell.ui.shell.ShellHeadItem')
        .filter(X => X.getProperty('icon')==='sap-icon://nav-back');
  ...
  pressButton(label) {
    ...
    if(buttons.length!=1) {
      let avaibale = this.getComponents("sap.m.Button").map(X => X.getText()).filter(T=>T.length);
  ...
  getPageCount() {
    let found = this.getObjectPageLayouts().concat(this.getDynamicPages())
  ...
  getCurrentPage() {
    let found = this.getObjectPageLayouts().concat(this.getDynamicPages())
      .filter(X => this.hasVisibleContainer(X), this);
    if(found.length!=1)
      throw Error(`getCurrentPage: exact page not found, length: ${found.length}`);
    return found[0];
  }
  ...
  listTableTitlesWithTotals() {
    ...
    return this.getComponentsByIdRe(re).map(X => X.getText()).filter(X=>X.length);
  }
  ...
  findTableByTitle(title) {
    ...
    if(title==='?')
      throw Error(`findTableByTitle - available: ${titles.map(T=>T[0]).join(', ')}`);
    let found = titles.filter(X=>X[0].startsWith(title));
    if(found.length==0)
      return this.findTableByTitleInSection(title);
    if(found.length!=1) {
      let available = titles.map(T=>T[0]);
      throw Error(`findTableByTitle(${title}): exact table not found, length: ${found.length}, available: ${available.join(', ')}`);
    }
    return found[0][1];
  }
  ...
  findTableByTitleInSection(title) {
    ...
    if(title==='?')
      throw Error(`findTableByTitleInSection - available: ${titles.map(T=>T[0]).join(', ')}`);
    let found = titles.filter(X=>X[0].startsWith(title));
    if(found.length!=1) {
      let available = titles.map(T=>T[0]);
      throw Error(`findTableByTitleInSection(${title}): exact table not found, length: ${found.length}, available: ${available.join(', ')}`);
    }
    return found[0][1];
  }
  ...
  getVisibleListReport() {
    let reports = this.getListReports()
      .filter(X => this.hasVisibleContainer(X));
    if(reports.length!=1)
  ...
  pressTile(name) {
    const found = this.getComponents("sap.m.GenericTile")
      .filter(X => X.getProperty("header")===name);
    if(found.length!==1)
      return "Tile not found, length:"+found.length;
  ...
  extractObjectPageContentWithBindingInfo() {
    ...
    function getBindings(X) {
      ...
      if(isCompositeBinding(binding))
        return binding.getBindings().map(X => X.getPath());
  ...
  extractTableContentWithBindingInfo() {
    ...
    function getBindings(X,p) {
      ...
      if(isCompositeBinding(binding))
        return binding.getBindings().map(Y => Y.getPath());
      return binding.getPath();
    }

    function extractData(X) {
      if(!X.mBindingInfos) return undefined;
      let info=Object.keys(X.mBindingInfos).map(name => {
  ...
  dumpObjectPageHeader(header) {
    ...
    return this.reduce(items.map(I => {
      return {
        title: I.getTitle(),
        data: this.reduce(I.getItems().map(II => {
          return this.reduce(II.getItems().map(III => {
  ...
  reduce(A) {
    if(typeof A==="object") {
      ...
      if(A.title && A.data && A.data.length==2 && A.title==A.data[0])
        return {[A.title]:A.data[1]};
    }
    return A;
  }
  ...
  dumpObjectPageHeaderTitle(headerTitle) { // sap.uxap.ObjectPageDynamicHeaderTitle
    let snappedHeading = this.dumpFormElementWrapperItems(headerTitle.getSnappedHeading().getItems());
    let snappedContent = headerTitle.getSnappedContent()
  ...
  extractSectionsContent(objectPage) { // sap.uxap.ObjectPageSection
    let R={};
  ...
  getFieldWrapperControls(oFieldWrapper) {
    return oFieldWrapper.getEditMode() === "Display" ? [oFieldWrapper.getContentDisplay()] : oFieldWrapper.getContentEdit();
  }
  ...
  dumpTableCellText(cell) {
    ...
    if(type == "sap.fe.macros.field.FieldAPI") {
      ...
      if(cdType=="sap.fe.core.controls.FormElementWrapper")
        return this.dumpFormElementWrapper(content);
      if(cdType=="sap.fe.core.controls.ConditionalWrapper")
        return this.getConditionalWrapperText(content);
      if(cdType=="sap.m.VBox")
        return this.dumpFormElementWrapperItems(content.getItems());
      if(cdType=="sap.m.HBox")
        return this.dumpFormElementWrapperItems(content.getItems());
    ...
    } else if(type=='sap.fe.macros.Field') {
  ...
  attachEventHandler() {
    ...
    sap.ui.getCore().attachEvent("UIUpdated", (e) => {window.allUIEvents+=1;});
    return true;
  }
  ...
  getListReportFilterBar() { // sap.fe.core.controls.FilterBar
    let listReport = this.getVisibleListReport();
    let items = listReport.getAggregation('header')
  ...
  tapOnComponentWithProperty(type, property, value, parent=false) {
    ...
    if(found.length!=1)
      throw Error(`tapOnComponent(${type},${property},${value}): exact component not found, length: ${found.length}`);
  ...
  selectAllFieldsInAdaptFilterDialog() {
    ...
    filterItems.map((_,row) => { // sap.ui.mdc.FilterField
  ...
  pressButtonInDialog(buttonText,dialogCaption) {
    ...
    if(buttons.length!=1)
      throw Error('Button not found, length: ' + buttons.length);
    return !!buttons[0].firePress();
  }
  ...
  findDialogByTitle(title) {
    let dialogs = this.getComponents('sap.m.Dialog')
      ...
      .filter(X => X.isActive());
    if(dialogs.length!=1)
      throw Error('Dialog not found, length: ' + dialogs.length);
    return dialogs[0];
  }
  ...
  findFieldInValueHelpDialog(fieldLabel) {
    ...
    if(found.length!=1) {
      let available = filterItems.map(X=>X.getLabel());
      throw Error(`selectRowInAdaptFilterDialog(${fieldLabel}) - no exact match, found: ${found.length}, avaibale: ${available.join(', ')}`);
    }
    return found[0];
  }
  ...
  findVisibleValueHelpDialogFilterBar() {
    ...
    if(found.length!=1) throw Error(`findVisibleValueHelpDialogFilterBar: FilterBar not found, length: ${found.length}`);
    return found[0];
  }
  ...
  pressButtonInActiveDialog( label ) {
    ...
    if(!button.getVisible()) throw Error(`pressButtonInActiveDialog(${label},${button.getId()}): button not visible`);
    return !!button.firePress();
  }

  extractErrorDialogContent(dialog) {
    return this.reduce(dialog.getContent()[0].getItems().map(I => {
  ...
  async pressViewDetailsInErrorDialog(dialog) {
    let links = this.reduce(dialog.getContent()[0].getItems().filter(I => I.getMetadata().getName()=='sap.m.Link'));
  ...
  findListItemInDialog(dialog, itemText) {
    ...
    function hasItemWithText(items, itemText) {
      return items.getContent()[0].getItems().filter(I => I.getText()===itemText).length>0;
    }

    let lists = dialog.getContent().filter(X => X.getMetadata().getName()==='sap.m.List');
    if(lists.length!=1)
      throw Error('List not found, length: ' + lists.length);
    let listItems = lists[0].getItems();
    let foundItem = listItems.filter(X => hasItemWithText(X,itemText))
    if(foundItem.length!=1)
      throw Error('Item not found, length: ' + foundItem.length);
    return foundItem[0];
  }
  ...
  chooseItemInListDialog(dialogName, itemText) {
    ...
    let buttons = dialog.getButtons().filter(B => B.getText()==='OK');
    if(buttons.length!=1)
      throw Error('Button not found, length: ' + foundItem.length);
    let buttonOK = buttons[0];
  ...
  findSearchFieldInFilterBar() {
    ...
    //sap.m.SearchField
    if(basicSearchField._getContent)
      return basicSearchField._getContent()[0];
  ...
  findShellSearchByIcon() {
    return this.getComponents('sap.ushell.ui.shell.ShellHeadItem')
      .filter(X => X.getProperty('icon') == 'sap-icon://search');
  }
  ...
  findGroupField(groupName, fieldLabel) {
    ...
    if(found.length!=1) throw Error(`Field ${fieldLabel} in group ${groupName} not found, got: ${found.length} elements`);
    return found[0];
  }
  ...
  getFormAPIs() {
    ...
    return this.getComponents('sap.fe.macros.form.FormAPI')
      .filter(X=>X.getId().startsWith(pageName));
  }
  ...
  getAllFormFontainerElements() {
    return this.getAllFormContainers().flat().map(X => X.getFormElements());
  }

  listFormContainerElementLabels() {
    return this.getAllFormFontainerElements().flat().map(X => X.getLabel());
  }
  ...
  findFormElementField(fieldLabel) {
    ...
    throw Error(`findFormElementField: one field ${fieldLabel} not found, length: ${found.length}, available: ${available.join(', ')}`);
  }
  ...
  findField(fieldLabel) {
    if(this.hasActiveDialog())
      return this.findFieldInValueHelpDialog(fieldLabel);
    try {
      let groupFields = this.listFieldGroupNames().map(F => {
        try {
  ...
  modifyField(fieldLabel, newText) {
    ...
    if(field.getMetadata().getName()=='sap.ui.layout.form.FormElement') {
      if(!this.hasAtLeastOneFieldInputWithoutValueHelp(fieldLabel))
        if(this.openValueHelpDialogForField(fieldLabel))
          return false;
    }
    if(field.getMetadata().getName()=='sap.ui.mdc.FilterField') {
      if(this.openValueHelpDialogForFilterField(fieldLabel))
        return false;
    }
  ...
  hasOpenedValueHelpPopover() {
    let found = this.getComponents('sap.ui.mdc.valuehelp.Popover')
      .filter(X => X.isOpen());
  ...
  findOpenedPopover() {
    let found = this.getComponents('sap.m.Popover')
      .filter(X => X.isOpen());
  ...
  dumpFormElementWrapperItems(items) {
    ...
    items.map(I => {
      ...
      if(type=="sap.m.ObjectIdentifier") {
        ...
        R.marker=I.getType();
        if(I.getAdditionalInfo())
          R.info=I.getAdditionalInfo();
  ...
  extractTableRows() {
    ...
    function getCellProperties(X) {
      ...
      let columnNames=columns.map(C => {
        ...
        if(header && header.isA("sap.ui.mdc.table.ColumnHeaderLabel"))
    ...
    return this.getComponents()
      .filter(isVisibleInnerTableRow)
      .map(getCellProperties);

  }

  getParentOfComponentWithProperty(type,property,value) {
    let components = this.getComponents(type)
      .filter(X => X.mProperties && X.mProperties[property]==value && X);
  ...
  getElementWithPropertyAndDump(property,value) {
    let elements=this.getComponents()
      .filter(X => X.mProperties && X.mProperties[property]==value && X);
  ...
  getVisiblePopoverCount() {
    return this.getComponents("sap.m.Popover")
      .filter(X=>X.oPopup.isOpen()).length;
  }
  ...
  findFilterBarFieldByLabel(label) {
    ...
    if(fields.length!=1) throw Error(`findFilterBarFieldByLabel(${label}) - exact matching field not found, length: ${fields.length}, available: ${available.join(', ')}`);
  ...
  findFilterBarField(name) {
    ...
    if(!field)
      field = this.findFilterBarFieldByLabel(name);
  ...
  findFieldValueHelp(name) {
    ...
    if(vh==null)
      vh = field.getId().replace('::FilterField::','::FilterFieldValueHelp::');
    let found = this.getComponentsById(vh)
    if(found.length!=1) throw Error(`findFieldValueHelp(${name}): Failed to find component, length: ${found.length}`);
    return found[0];
  }

  selectItemInValueHelp(fieldName, text, tap=false) {
    if(this.hasOpenedValueHelpPopover())
      return this.selectItemInValueHelpPopup(fieldName, text, tap);
    if(this.hasActiveDialog())
      return this.selectOneRowInValueHelpDialog(0, text);
  ...
  selectItemInValueHelpPopup(fieldName, text, tap=false) {
    ...
    try {
      fieldFieldValueHelpId = field.getFields()[0].getContent().contentEdit[0].getFocusElementForValueHelp().getId();
    } catch(e) {
    }
    ...
    popoverFieldValueHelpId = popoverFieldValueHelpId.replace('::FilterFieldValueHelp::','::FilterField::').replace('::FieldValueHelp::', '::FormElement::DataField::');

    if(!fieldFieldValueHelpId.startsWith(popoverFieldValueHelpId))
      throw Error(`selectItemInValueHelpPopup() - id mismatch ${fieldFieldValueHelpId}!=${popoverFieldValueHelpId}`);
  ...
  selectItemInFieldValueHelp(fieldName, text) {
    ...
    if(type=='sap.ui.mdc.ValueHelp')
      return this._selectItemInValueHelp(valueHelp, text);
    if(type=='sap.ui.mdc.field.ListFieldHelp')
      return this._selectItemInValueHelpList(valueHelp, text);
  ...
  _selectItemInValueHelpList(valueHelp, text) {
    ...
    if(found.length!=1) throw Error(`_selectItemInValueHelpList(${text}): item not found, length: ${found.length}`);
    let item = found[0];
    return !!$(this.jqid(item.getId())).tap();
  }

  _selectItemInValueHelp(valueHelp, value) { // sap.ui.mdc.ValueHelp
    ...
    if(content.isA('sap.ui.mdc.valuehelp.content.FixedList'))
      ...
      this._selectItemInTable(content.getTable(), value);
  }

  _selectItemInList(list, text) { // sap.m.List
    let items = list.getItems();
    let found = items.filter(X=>X.getLabel()==text);
    if(found.length!=1) {
      let available = items.map(X=>X.getLabel());
      throw Error(`_selectItemInList: Failed to find one matching item, length: ${found.length}, available: ${available.join(', ')}`);
    }
    let listItem = found[0];
    list.fireItemPress({listItem});
  }

  _selectItemInFixedList(list, text) { // sap.ui.mdc.valuehelp.content.FixedList
    let items = list.getItems();
    let index = items.findIndex(X=>X.getText()==text);
    if(index==-1) {
      let available = items.map(X=>X.getText());
      throw Error(`_selectItemInFixedList: Failed to find one matching item, length: ${found.length}, available: ${available.join(', ')}`);
    }
  ...
  _selectItemInTable(table, value, tap=false) {
    if(!table.isA('sap.m.Table')) throw Error(`_selectItemInTable(${table.getId()}): expected: sap.m.Table, actual: ${table.getMetadata.getName()}`);
    ...
    if(!value) {
      items.forEach(item => {
        ...
        if(checkbox)
          ...
          throw Error(`_selectItemInTable: Failed to remove selection - missing MultiSelectControl`);
      })
      return;
    }
    let found = items.map((I,index) => {
      let texts = I.getCells().filter((C,I)=>columnVisible[I]).map(this.dumpTableCellText,this);
      return texts.filter(T=>T==value).length==0?-1:index;
    }, this).filter(X=>X>=0);
    if(found.length!=1) throw Error(`_selectItemInTable: Failed to find one matching row, length: ${found.length}`);
    let item = items[found[0]];
    let checkbox = item.getMultiSelectControl();
    if(checkbox && !tap)
      checkbox.fireSelect({selected:true});
    else
      this.tapOnAnyId(item.getId());
  }

  _getColumnIndexByNameInMdcTable(mdcTable, columnName) {
    let columns = mdcTable.getColumns();
    if(typeof columnName==='number') return columnName;
    let found = columns.map((C,I) => {
  ...
  _selectRowInValueHelpDialog(rowIndex) {
    ...
    if(!this.getTableUtils().hasRowHeader(table)) { // the selector is hidden
      let row = table.getRows()[rowIndex];
      if(this.getVersionInt()<116)
        $(row.getDomRef('col0')).click();
      else
        $(row.getDomRef('col0')).tap();
    } else {
      if(this.getVersionInt()<116)
        $(table.getDomRef('rowsel'+rowIndex)).click();
      else
        $(table.getDomRef('rowsel'+rowIndex)).tap();
    }
  }
  ...
  findRowIndexInValueHelpDialog(columnName, searchText, firstMatching=false) {
    ...
    if(firstMatching && found.length>0)
      return found[0];
  ...
  hasFieldInputWithValueHelp(fieldName) { // price+currency
    let field = this.findField(fieldName);
    try {
  ...
  hasAtLeastOneFieldInputWithoutValueHelp(fieldName) { // price+currency
    let field = this.findField(fieldName);
    try {
      let edit = field.getFields()[0].getContent().getContentEdit()[0];
      let content = edit.getAggregation('_content');
  ...
  openValueHelpForFieldInput(fieldName) { // price+currency
    let field = this.findField(fieldName);
    try {
      let edit = field.getFields()[0].getContent().getContentEdit()[0];
      ...
      let content = edit.getAggregation('_content');
      let vhFields = content.filter(c=>c.getShowValueHelp());
      if(vhFields.length!=1) throw Error(`openValueHelpForFieldInput(${fieldName}): missing FieldInput with showValueHelp`);
  ...
  hasValueHelp(fieldName) {
    if(this.hasFieldInputWithValueHelp(fieldName))
      return true;
    let field = this.findField(fieldName);
  ...
  openValueHelpDialogForField(fieldName) {
    if(this.hasFieldInputWithValueHelp(fieldName))
      return this.openValueHelpForFieldInput(fieldName);
    if(this.hasActiveDialog())
      return this.openValueHelpDialogForDialogFilterField(fieldName);
    else if(this.getVisibleObjectPage())
      return this.openValueHelpDialogForObjectField(fieldName);
    else
      return this.openValueHelpDialogForFilterField(fieldName);
  }
  ...
  getPageName() {
    ...
    if(!pageName)
      throw Error('Failed to guess the name of the page');
    return pageName;
  }
  ...
  extractTableRowsTotalCount(tableName) {
    ...
    if(filtered.length!=1) {
      let re=/(.*)( \([\d\.\,]*\))?/;
      let available = tablesWithTotals.map(X => X.match(re)).filter(X=>X).map(X=>X[1]);
      throw Error(`extractTableRowsTotalCount(${tableName}) - table not found, available: ${available.join(', ')}`);
    }
    let rext = /\(([\d\.\,]*)\)/;
  ...
  extractCurrentPageContent() {
    ...
    if(type==="sap.f.DynamicPage")
      return this.extractDynamicPageContent(page.getContent());
    if(type==="sap.uxap.ObjectPageLayout") {
      if(page.getContent)
        return this.extractObjectPageContent(page.getContent());
      if(page.getSections)
      return this.extractSectionsContent(page);
    }
  ...
  extractKnownComponentsContent(component) {
    ...
    if(type==="sap.ui.table.TreeTable")
      return this.extractTreeTableContent(component);
    return undefined;
  }
  ...
  extractTreeTableContent(table) {
    let rows = table.getRows();
    return rows
      .map(R => R.getCells().map(C => C.getText()))
      .filter(V => V[0] || V[1]);
  }
  ...
  findObjectPageFooterBarMessageButton() {
    ...
    return this.getComponents('sap.fe.macros.messages.MessageButton')
      .filter(X => X.sId==id)[0];
  }
  ...
  getInbounds() {
    ...
    if(!ushellConfig)
      throw Error("No ushell config found!");
