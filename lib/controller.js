...
class Controller {
  ...
  constructor(process) {
    this.ui5Loaded = false;
    this.process = process;
  }

  getWorkingDirectory() {
    this.workingDirectory = this.process?.params.workingDirectory
    if(!this.workingDirectory) {
      const createWorkingDirectory = require('./createWorkingDirectory')
  ...
  async initDriver() {
    ...
    if(CDS_CUCUMBER_BROWSER) {
      if(SupportedBrowsers.includes(CDS_CUCUMBER_BROWSER))
        this.browser=CDS_CUCUMBER_BROWSER;
      else
        throw Error(`Browser not supported: ${CDS_CUCUMBER_BROWSER}, available: ${SupportedBrowsers.join(', ')}`);
    } else {
    ...
    if(env.ENABLE_CORS==="1")
      options.addArguments('--disable-web-security')
    if(env.ACCEPT_LANG)
      options.addArguments('--accept-lang='+env.ACCEPT_LANG)
    process.env.TMPDIR = this.getWorkingDirectory();
    let builder = await new Builder()
      .forBrowser(this.browser);
    if(this.usingFirefox())
  ...
  async initializeLibrary() {
    try {
      if(env.CDS_CUCUMBER_DEBUG=="1")
        ...
        return await this.injectLibrary();
    } catch(e) {
      if(env.SLOW_QUIT)
        await this._sleep(1000*parseInt(env.SLOW_QUIT));
      throw e;
    }
  }
  ...
  async injectLibrary() {
    ...
    this.ui5Loaded=rc!=='nix';
    if(!this.ui5Loaded) throw Error('injectLibrary failed: SAPUI5 not loaded');
    if(!await this.attachEventHandler())
      throw Error("injectLibrary: attachEventHandler failed");
    if(env.SAP_UI5_VERSION) {
  ...
  async loadLibrary() {
    ...
    this.ui5Loaded=rc!=='nix';
    if(!this.ui5Loaded) throw Error('loadLibrary failed: SAPUI5 not loaded');
    await this.waitForWindowCDS();
    if(!await this.attachEventHandler())
      throw Error('loadLibrary: attachEventHandler failed');
    if(env.CDS_CUCUMBER_WAIT_DEBUGGER=="1")
      await this.waitDebuggerToAttach();
    return rc;
  }
  ...
  async openUrl(url, language=undefined) {
    this.url = url;
    if(language)
      return await this.driver.get(url+`?sap-language=${language}`);
  ...
  async clickOnElementWithValue(value) {
    let element = await this.driver.findElement(By.xpath("//*[@value='"+value+"']"));
    let actions = this.driver.actions({async: true});
    await actions.move({origin:element}).click().perform();
    return {value,elementId:await element.getId()};
  }
  ...
  async clickOnElementWithText(text) {
    ...
    let actions = this.driver.actions({async: true});
    await actions.move({origin:element}).click().perform();
    return {value: text,elementId:await element.getId()};
  }

  async existsElementWithText(text) {
    let element = await this.driver.findElement(By.xpath("//*[text()='"+text+"']"))
    if(!element) throw "existsElementWithText: element not found - "+text;
  }

  async clickOnElementWithLinkText(text) {
    await this.driver.findElement(By.linkText(text)).click();
  }

  async enterTextInElementWithId(text, id) {
    let element = await this.driver.findElement(By.id(id));
    await element.sendKeys(text);
  }

  async clickInElementWithId(id) {
    await this.driver.executeScript("document.getElementById('"+id+"').focus();");
    await this.driver.executeScript("document.getElementById('"+id+"').click();");
  }
  ...
  async _call(name,...args) {
    ...
    if(env.SLOW_DOWN && name!="getEvents")
      await this._sleep(1000*parseInt(env.SLOW_DOWN));
    if(this.expectToFail && name!="getEvents")
      return await this._executeFailingScript(src);
  ...
  async _executeScript(src) {
    ...
    if(typeof result == "string" && result.indexOf('Exception: ')==0)
      throw Error(result);
    return result;
  }
  ...
  async checkForErrors() {
    ...
    if(error) {
      try { messages = await this.extractFooterBarMessages(); } catch(e) {}
