...
function createWorkingDirectory() {
  ...
  let tmpdir = isUnixOS
    ?
      path.join(env.CDS_CUCUMBER_TMPDIR||'/tmp', `cds_cucumber_${os.userInfo().username}`, `${process.pid}_${date}`)
    :
      path.join(env.CDS_CUCUMBER_TMPDIR||'c:\\temp', `cds_cucumber_${os.userInfo().username}_${process.pid}_${date}`)
    ;
