...
function infoProcess(process) {
  if(process.pid===0)
    console.log(`Connected to process '${process.params?.stack||"node"}'`);
  else
    console.log(`Started process '${process.params.stack}' with pid ${process.pid}`);
}
...
Given('we have started the application', async function() {
  ...
  this.allApplications['instance#'+this.allProcesses.length]=this.process;
  infoProcess(this.process);
  await loadModel();
})

When('we switch to application {string}', async function(application) {
  this.process = this.allApplications[application]
  infoProcess(this.process);
})

When('we switch to app instance {int}', async function(instance) {
  this.process = this.allApplications['instance#'+instance]
  infoProcess(this.process);
})
...
Given('we have started application {string}', async function(application) {
  ...
  this.allProcesses.push(this.process);
  this.allApplications['instance#'+this.allProcesses.length]=this.process;
  this.allApplications[application]=this.process;
...
Given('we have started the java application', async function() {
  ...
  this.allProcesses.push(this.process);
  this.allApplications['instance#'+this.allProcesses.length]=this.process;
  this.allApplications['java']=this.process
...
Given('we have started the node application', async function() {
  ...
  this.allProcesses.push(this.process);
  this.allApplications['instance#'+this.allProcesses.length]=this.process;
  this.allApplications['node']=this.process
  infoProcess(this.process);
  await loadModel();
})
...
* @param {string} endpoint endpoint to use for authentication
* @description Opens the specified url in the browser and uses the specified username to login
* @example Given we login with user "alice" and password "admin" via "/"
*/
...
async function openUrl(controller, process, url, options) {
  ...
  try {
    ...
    if(isUI5url) {
      ...
      if(!username && protectedOdataEndpoint && protectedOdataEndpoint!="shell")
    ...
    if(username && isUI5url) {
      if(protectedOdataEndpoint && !isShell) {
...
async function inspectUrl(process, url, username, password) {
  url = buildUrl(process, url);
  const httpGet = url.startsWith('https')?require('../../httpsGet'):require('../../httpGet')
