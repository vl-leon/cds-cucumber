...
When('we select tile {string}', async function(tile) {
  ...
  if(message)
    throw Error(`Found unexpected error message: ${JSON.stringify(message)}`);
})
...
When('we click on html link {string}', async function(text) {
  await this.controller.clickOnElementWithLinkText(text)
})
