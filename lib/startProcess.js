...
async function startProcess(params={}) {
  ...
  if(env.CUCUMBER_CDS_PROFILE==='hybrid') {
    args = ['cds','bind','--exec','--'].concat(args).concat(['--profile',env.CUCUMBER_CDS_PROFILE])
  }
  ...
  if(isJava)
    ...
    lookupString = [/\[cds\] - launched at /, /\[cds\] - server .* launched in/];
  let loops = 2000;
  for(var i=0; i<loops; i++) {
    ...
    await sleep(500);
    let found = lookupString.map(re => cdsProcess.scriptOutput.match(re)).filter(r=>r).map(r=>r[0])
    if(found.length > 0) {
      process.stdout.write('\n');
      break;
    }
...
if(require.main == module) {
  (async function() {
    try {
      ...
      throw Error(`Failed to start process, error: ${error}`);
    }
  })();
} else {
  module.exports = startProcess;
